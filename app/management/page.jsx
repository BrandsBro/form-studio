...
export default function ManagementLogin(){
  ...
  async function handleLogin(){
    ...
    }catch(e){setErr("Error connecting. Please try again.");setLoading(false);}
  }
  ...
  return(
    <div style={{minHeight:"100vh",background:"#0D1117",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"space-between",minHeight:"100vh",padding:20,fontFamily:"var(--font-dm-sans)",overflow:"hidden",position:"relative"}}>
      <style>{`
        ...
        @keyframes particleFade{0%{opacity:0.8;transform:scale(1)}100%{opacity:0;transform:scale(0) translateY(-50px)}}
      ...
      <div style={{position:"relative",zIndex:1,width:"min(420px,100%)",animation:mounted?"fadeUp 0.7s ease":"none",opacity:mounted?1:0}}>
        <div style={{textAlign:"center",marginBottom:36}}>
          <div style={{position:"relative",display:"inline-block",marginBottom:20}}>
            ...
            <div style={{width:64,height:64,borderRadius:18,background:"linear-gradient(135deg,rgba(245,158,11,0.2),rgba(245,158,11,0.08))",border:"1px solid rgba(245,158,11,0.3)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:28,animation:"float 3s ease-in-out infinite, pulse 3s ease-in-out infinite",position:"relative"}}>⭐</div>
          </div>
          <h1 style={{color:"white",fontSize:28,fontWeight:800,margin:"0 0 8px",fontFamily:"var(--font-playfair)",background:"linear-gradient(135deg,#fff,#F59E0B,#fff)",backgroundSize:"200% auto",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",animation:"gradientShift 4s ease infinite"}}>Performance Reviews</h1>
        ...
        <div style={{background:"linear-gradient(180deg,#161B22,#12181F)",border:"1px solid #21262D",borderRadius:16,padding:28,boxShadow:"0 32px 80px rgba(0,0,0,0.5)"}}>
          ...
          <input value={email} onChange={e=>{setEmail(e.target.value);setErr("");}} onKeyDown={e=>e.key==="Enter"&&handleLogin()}
            placeholder="your@email.com" type="email" autoFocus
            style={{width:"100%",background:"#0D1117",border:"1px solid "+(err?"rgba(239,68,68,0.5)":"#21262D"),borderRadius:10,padding:"12px 16px",color:"white",fontSize:14,outline:"none",boxSizing:"border-box",marginBottom:err?8:16,transition:"border-color 0.2s"}}
            onFocus={e=>e.target.style.borderColor="#F59E0B"} onBlur={e=>e.target.style.borderColor=err?"rgba(239,68,68,0.5)":"#21262D"}/>
          {err&&<div style={{display:"flex",alignItems:"center",gap:8,padding:"10px 14px",background:"rgba(239,68,68,0.08)",border:"1px solid rgba(239,68,68,0.2)",borderRadius:8,marginBottom:14}}><span style={{color:"#ef4444"}}>⚠️</span><p style={{color:"#ef4444",fontSize:12,margin:0}}>{err}</p></div>}
          <button onClick={handleLogin} disabled={loading}
            ...
            {loading?<><svg style={{width:16,height:16,animation:"spin 1s linear infinite"}} viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" opacity="0.25"/><path fill="currentColor" d="M4 12a8 8 0 018-8v8H4z"/></svg>Finding your forms...</>:<>Continue <ChevronRight size={18}/></>}
          </button>
        </div>
