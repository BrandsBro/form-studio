...
export default function Login(){
  ...
  async function handleLogin(){
    ...
    try{
      ...
      const person=people.find(p=>p.email.toLowerCase().trim()===email.toLowerCase().trim()&&p.password&&p.password===password);
      if(!person){
  ...
  return(
    <div style={{minHeight:"100vh",background:"#0D1117",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:20,fontFamily:"var(--font-dm-sans)",overflow:"hidden",position:"relative"}}>
      <style>{`
        ...
        @keyframes particleFade{0%{opacity:0.8;transform:scale(1) translateY(0)}100%{opacity:0;transform:scale(0) translateY(-50px)}}
      ...
      {/* Twinkling stars */}
      {stars.map(s=>(
        <div key={s.id} style={{position:"fixed",left:s.x+"%",top:s.y+"%",width:s.size,height:s.size,borderRadius:"50%",background:"white",opacity:0.3,pointerEvents:"none",animation:`twinkle ${s.dur}s ease-in-out ${s.delay}s infinite`}}/>
      ))}

      {/* Floating particles */}
      {particles.map(p=>(
        <div key={p.id} style={{position:"fixed",left:p.x+"%",top:p.y+"%",width:5,height:5,borderRadius:"50%",background:p.color,pointerEvents:"none",animation:"particleFade 2s ease-out forwards",boxShadow:`0 0 8px ${p.color}`}}/>
      ))}
      ...
      <div style={{position:"relative",zIndex:1,width:"min(420px,100%)",animation:mounted?"fadeUp 0.7s ease":"none",opacity:mounted?1:0}}>
        ...
        <div style={{textAlign:"center",marginBottom:36}}>
          ...
          <div style={{position:"relative",display:"inline-block",marginBottom:20}}>
            {[0,1,2].map(i=>(
              <div key={i} style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",width:56+i*24,height:56+i*24,borderRadius:"50%",border:"1px solid rgba(245,158,11,0.15)",animation:`ringPulse ${2+i*0.7}s ease-out ${i*0.4}s infinite`}}/>
            ))}
            <div style={{width:64,height:64,borderRadius:18,background:"linear-gradient(135deg,rgba(245,158,11,0.2),rgba(245,158,11,0.08))",border:"1px solid rgba(245,158,11,0.3)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:28,animation:"float 3s ease-in-out infinite, pulse 3s ease-in-out infinite",position:"relative"}}>
              ⭐
            </div>
          </div>
          <h1 style={{color:"white",fontSize:28,fontWeight:800,margin:"0 0 8px",fontFamily:"var(--font-playfair)",background:"linear-gradient(135deg,#fff,#F59E0B,#fff)",backgroundSize:"200% auto",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",animation:"gradientShift 4s ease infinite"}}>
            Performance Reviews
          </h1>
        ...
        <div style={{background:"linear-gradient(180deg,#161B22,#12181F)",border:"1px solid #21262D",borderRadius:16,padding:28,boxShadow:"0 32px 80px rgba(0,0,0,0.5)"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:16}}>
            ...
            {err&&(
              <div style={{display:"flex",alignItems:"center",gap:8,padding:"10px 14px",background:"rgba(239,68,68,0.08)",border:"1px solid rgba(239,68,68,0.2)",borderRadius:8}}>
            ...
            <button onClick={handleLogin} disabled={loading}
              style={{width:"100%",padding:"13px 0",borderRadius:10,border:"none",background:loading?"#374151":"linear-gradient(135deg,#D97706,#F59E0B)",color:loading?"#9ca3af":"#000",fontSize:14,fontWeight:700,cursor:loading?"not-allowed":"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginTop:4,transition:"transform 0.1s,box-shadow 0.2s"}}
        ...
        <p style={{textAlign:"center",color:"#374151",fontSize:11,marginTop:16,fontFamily:"monospace"}}>FormStudio · Secure Access</p>
      </div>
      {/* Footer */}
