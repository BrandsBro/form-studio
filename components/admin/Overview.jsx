...
function Av({ name = "", size = 32 }) {
  const color = gc(name);
  return (
    <div style={{ width: size, height: size, borderRadius: "50%", background: color + "18", border: "2px solid " + color + "44", display: "flex", alignItems: "center", justifyContent: "center", fontSize: size * 0.33, fontWeight: 700, color, flexShrink: 0 }}>
      {gi(name)}
    </div>
  );
}
...
function getFormColor(form) {
  ...
  return form?.customColor || T[form?.theme] || "#F59E0B";
}
...
export default function Overview() {
  ...
  useEffect(() => {
    if (selectedId) {
      setLoadingSubs(true);
      getSubmissions(selectedId).then(s => {
        setAllSubs(s);
